...
### 5.1 지원자 1 — Project Lead / Control & RL

저는 프로젝트의 전체 방향 설정과 **강화학습 기반 보행 제어 파트**를 담당하고 있습니다.
...
현재는 특히 **강화학습 알고리즘 자체보다, 실제 로봇에 적용 가능한 보행 policy를 만들기 위해 어떤 관측값, 행동 공간, 보상함수, 평가 지표가 필요한지**를 중심으로 공부하고 있습니다.

### 5.2 지원자 2 — System Design / Experiment Design

두 번째 지원자는 프로젝트 내에서 **시스템 설계와 실험 구조 설계**를 담당하고 있습니다.

여기서의 설계는 Mechanical CAD 설계가 아니라, **로봇 제어 시스템이 어떤 구조로 연결되어야 하는지, 시뮬레이션과 실물 실험을 어떻게 구성할지에 대한 설계**입니다.

주요 역할은 다음과 같습니다.

- 전체 로봇 제어 흐름 정리
- 시뮬레이션 실험 구조 설계
- rigid foot 모델과 SoftFoot 모델 비교 실험 방향 정리
- 학습 결과 분석 방식 정리
- 센서 데이터와 평가 지표 연결 구조 설계
- 실험 로그 및 결과 정리 방식 설계
- 프로젝트 문서화 및 기술 자료 정리

이 역할은 프로젝트가 단순히 기능 구현에 그치지 않고, **실험 가능한 로보틱스 프로젝트**가 되도록 구조화하는 역할입니다.

### 5.3 지원자 3 — Software / Embedded Development
...
이 역할은 시뮬레이션에서 얻은 policy와 실제 로봇 하드웨어 사이를 연결하는 역할입니다.
...
저희가 현재 진행 중인 프로젝트는 아직 완성 단계는 아니지만, 다음 점에서 OH! GYM과 직접적으로 연결됩니다.
